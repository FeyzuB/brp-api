...
- Service procedure accepts minimal “safe” inputs (or accepts i_request_json too), prepares the JSON request body, calls the API procedure, then parses the response JSON into typed objects using Oracle JSON types (JSON_OBJECT_T / JSON_ARRAY_T).
